...
export function TestimonialsBlock({
  ...
  return (
    <section className={`${paddingTop} ${paddingBottom} px-4 max-w-6xl mx-auto space-y-10`}>
      {/* HEADER SECTION */}
      {(title || subtitle) && (
        <Reveal animation="fade-down" className="text-center space-y-2 max-w-2xl mx-auto">
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          {items.length > 1 && (
            <div className="flex items-center justify-between mt-6 px-2">
              ...
              <div className="flex items-center gap-2">
                <Button variant="outline" size="icon" className="btn-hover-effect rounded-full size-10 border-slate-300 dark:border-slate-700 shadow-sm hover:shadow-md transition-all duration-300" onClick={handlePrev}>
                  <ChevronLeft className="size-5" />
                </Button>
                <Button variant="outline" size="icon" className="btn-hover-effect rounded-full size-10 border-slate-300 dark:border-slate-700 shadow-sm hover:shadow-md transition-all duration-300" onClick={handleNext}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item, idx) => (
            <Reveal
              ...
              className="h-full"
            >
              <div className="card-hover-effect bg-[var(--theme-card-bg,rgba(255,255,255,0.85))] dark:bg-slate-900/80 backdrop-blur-md border border-[var(--theme-card-border,rgba(226,232,240,0.8))] dark:border-slate-800 rounded-3xl p-7 shadow-sm flex flex-col justify-between space-y-6 h-full">
