        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 shadow-sm">
            <span className="text-xs font-bold px-3 py-1 bg-indigo-50 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400 rounded-full">
              {product.type === "VIRTUAL_COURSE" ? "Virtual Course" : "Digital Product"}
            </span>
            <h1 className="text-4xl font-extrabold text-slate-900 dark:text-white mt-4">{product.title}</h1>
            <p className="text-slate-600 dark:text-slate-400 mt-4 leading-relaxed whitespace-pre-line">
          ...
          <div className="sticky top-8 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-6">
            ...
            <div>
              ...
              <div className="text-3xl font-black text-slate-900 dark:text-white">
                ${product.price.toLocaleString()} <span className="text-sm font-semibold">{product.currency}</span>
              </div>
            </div>

            <Link
              href={`/checkout/${product.id}`}
              className="w-full block text-center py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl transition shadow-lg shadow-indigo-600/20"
            >
              Enroll Now
            </Link>
